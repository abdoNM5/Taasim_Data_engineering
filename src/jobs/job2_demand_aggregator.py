...
gps_kafka_source = KafkaSource.builder() \
    ...
    .set_group_id("taasim-job2-demand-gps") \
...
trips_kafka_source = KafkaSource.builder() \
    ...
    .set_group_id("taasim-job2-demand-trips") \
    .set_starting_offsets(KafkaOffsetsInitializer.earliest()) \
    .set_value_only_deserializer(SimpleStringSchema()) \
    .build()
...
kafka_sink = KafkaSink.builder() \
    ...
    .build()
windowed_aggregated.map(tuple_to_demand_json, output_type=Types.STRING()).sink_to(kafka_sink).name("Kafka Sink")
