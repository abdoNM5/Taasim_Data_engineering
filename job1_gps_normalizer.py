...
kafka_props = {
    ...
    'group.id': 'taasim-job1-gps',
    'auto.offset.reset': 'earliest'
}

raw_stream = env.add_source(FlinkKafkaConsumer('raw.gps', SimpleStringSchema(), properties=kafka_props))
...
kafka_sink = FlinkKafkaProducer("processed.gps", SimpleStringSchema(), producer_config={"bootstrap.servers": "127.0.0.1:9094", "transaction.timeout.ms": "900000"})
anonymized_stream.map(tuple_to_json, output_type=Types.STRING()).add_sink(kafka_sink)
...
CassandraSink.add_sink(anonymized_stream).set_host("127.0.0.1", 9042).set_query(cassandra_query).build()
