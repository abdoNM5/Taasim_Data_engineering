...
def _import_kafka() -> tuple[Any, Any, Any]:
    try:
        ...
        return KafkaProducer, KafkaConsumer, NoBrokersAvailable
    except ImportError as exc:
        raise RuntimeError("kafka-python is required to use Kafka features") from exc


def create_producer(config: AppConfig, retries: int = 10, delay: float = 3.0) -> Any:
    KafkaProducer, _, NoBrokersAvailable = _import_kafka()
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            logger.warning("Kafka producer attempt %d/%d failed: %s", attempt, retries, exc)
            if attempt < retries:
                time.sleep(delay)
    
    raise RuntimeError(f"Kafka producer could not connect after {retries} attempts")


def create_consumer(config: AppConfig, retries: int = 10, delay: float = 3.0) -> Any:
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            logger.warning("Kafka consumer attempt %d/%d failed: %s", attempt, retries, exc)
            if attempt < retries:
                time.sleep(delay)

    raise RuntimeError(f"Kafka consumer could not connect after {retries} attempts")
